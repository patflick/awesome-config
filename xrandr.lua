...
function xrandr.menu()
    ...
    -- add a `copy` display (ie, --same-as) option that copies all outputs
    if #out > 1 then
        local cmd = "xrandr"
        for i, o in pairs(out) do
            cmd = cmd .. " --output " .. o .. " --auto"
            if i > 1 then
                cmd = cmd .. " --same-as " .. out[i-1]
            end
        end
        local label = "["
        for i, o in pairs(out) do
...
function xrandr.switch(timeout)
    ...
    if not state.iterator then
        state.iterator = awful.util.table.iterate(xrandr.menu(), function()
            return true
        end)
    end
